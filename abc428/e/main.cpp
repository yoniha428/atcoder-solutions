...
struct Rerooting {
    ...
    struct DP {  // DP の型
        pair<int, int> dp;
        DP(int v, int i) : dp(make_pair(v, i)) {};
        DP(pair<int, int> p) : dp(p) {};
    };
    const DP identity = DP(-1, 0);  // 単位元(末端の値は add_root(identity) になるので注意)
    function<DP(DP, DP)> merge = [](DP dp_cum, DP d) -> DP {
        return DP(max(dp_cum.dp, d.dp));
    };
    function<DP(DP)> add_root = [](DP d) -> DP {
        return DP(d.dp.first + 1, d.dp.second);
    };
    /* end 問題ごとに書き換え */
    // グラフの定義
    struct Edge {
...
signed main(){
  ...
  reroot.build();
  for(auto dp : reroot.ans) cout << dp.dp.second + 1 << '\n';
}
